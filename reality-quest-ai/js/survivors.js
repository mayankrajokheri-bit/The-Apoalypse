...
// Uses avatar portrait as head + colored torso + legs for perfect alignment
function renderInGameSurvivorSprite(containerId, options = {}) {
  ...
  // Pick a torso color per character role
  const torsoColors = {
    aarav: 'linear-gradient(180deg,#3498db,#1a5276)',
    alex:  'linear-gradient(180deg,#e67e22,#784212)',
    ben:   'linear-gradient(180deg,#f1c40f,#7d6608)',
    chloe: 'linear-gradient(180deg,#1abc9c,#0e6655)',
    dev:   'linear-gradient(180deg,#e74c3c,#6e1b12)',
    esha:  'linear-gradient(180deg,#2ecc71,#145a32)',
    felix: 'linear-gradient(180deg,#00cec9,#004d4b)',
    grace: 'linear-gradient(180deg,#9b59b6,#4a235a)',
    henry: 'linear-gradient(180deg,#d35400,#641e16)',
    tara:  'linear-gradient(180deg,#fd79a8,#7d2447)'
  };
  const torso = torsoColors[surv.id] || 'linear-gradient(180deg,#2e86de,#1b4f72)';

  container.innerHTML = `
    <div class="in-game-survivor-model" id="in-game-model-${surv.id}">
      <div class="player-hud-tag" style="font-size:9px; font-weight:800; letter-spacing:1px; white-space:nowrap; margin-bottom:2px; background:rgba(0,0,0,0.75); padding:1px 6px; border-radius:4px; border:1px solid #00ff9d; color:#fff; position:relative; top:0; left:0; transform:none;">
        ${name}
      </div>
      <div style="display:flex; flex-direction:column; align-items:center;">
        <div class="survivor-avatar ${surv.avatarClass}" style="width:26px; height:26px; border-width:2px; box-shadow:0 0 8px rgba(0,255,157,0.7); flex-shrink:0;"></div>
        <div style="width:24px; height:26px; background:${torso}; border-radius:4px; margin-top:1px;"></div>
        <div style="display:flex; gap:4px; margin-top:1px;">
          <div style="width:6px; height:14px; background:#111; border-radius:2px;"></div>
          <div style="width:6px; height:14px; background:#111; border-radius:2px;"></div>
        </div>
      </div>
